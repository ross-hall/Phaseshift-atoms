 ...
 *   1. Raw material squares travel into a red laser line.
 *   2. Past the laser, a single continuous sheet (the melted, assembled
 *      metal) flows on into a grey line.
 *   3. The grey line is a die: a finished part emerges from it and
 ...
 * from elapsed time and a spawn interval, so only a small, bounded window
 * of in-flight items is ever iterated (no ever-growing arrays), which
 * keeps it cheap to run indefinitely.
 *
 * Each of the three stages can show a custom image in place of its
 * default wireframe shape (see `imageSlots` / setImage / clearImage).
 */
class AssemblyAnimation {
  constructor(canvas) {
    ...
    this.schema = [
      ...
      { key: 'squareInterval', label: 'Material Feed Interval (ms)', min: 150, max: 3000, step: 50 },
      { key: 'partSize', label: 'Part Size', min: 10, max: 70, step: 1 },
      { key: 'partInterval', label: 'Part Output Interval (ms)', min: 150, max: 3000, step: 50 },
      ...
      { key: 'gateHeight', label: 'Laser/Die Height', min: 40, max: 160, step: 2 },
  ...
  render(elapsed) {
    ...
    // --- Sheet (continuous, between laser and die line) ---
    ...
    // --- Raw material squares feeding into the laser ---
    const squarePositions = this._streamPositions(
      elapsed, p.squareInterval, 0, speedPxMs, -p.squareSize, laserX
    );
    for (const x of squarePositions) this._drawSquare(x, beltY, p.squareSize);
    ...
    // --- Laser line (red, pulses each time a square reaches it) ---
    const travelToLaser = (laserX - -p.squareSize) / speedPxMs;
    const laserPulse = this._pulse(elapsed, p.squareInterval, travelToLaser, p.pulseDuration);
    const laserGlow = 0.55 + 0.45 * Math.sin(elapsed * 0.006) * 0.5 + laserPulse * 0.5;
    ctx.strokeStyle = `rgba(255,45,45,${clamp(laserGlow, 0.4, 1)})`;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(laserX, beltY - p.gateHeight / 2);
    ctx.lineTo(laserX, beltY + p.gateHeight / 2);
    ctx.stroke();
    if (laserPulse > 0) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(greyX, beltY - p.gateHeight / 2);
    ctx.lineTo(greyX, beltY + p.gateHeight / 2);
    ctx.stroke();
    if (diePulse > 0) {
